...
var questions = [
  ...
  },
  {
    ...
    text: "A cluster has three nodes labeled `zone=us-east-1a`, `zone=us-east-1b`, and `zone=us-east-1c`. A Deployment with 3 replicas specifies `topologySpreadConstraints` with `maxSkew: 1`, `topologyKey: zone`, `whenUnsatisfiable: DoNotSchedule`, and `nodeTaintsPolicy: Ignore`. Node `us-east-1c` becomes `NotReady`, but the existing pod on that node has not yet been evicted. When the Deployment is immediately scaled to 6 replicas, how are the new pods distributed?",
    diagram: null,
    options: [
  ...
  },
  {
    ...
    diagram: null,
    options: [
      "A. Raise the alert threshold: change from 5% to 10% to account for statistical insignificance during low-traffic deployment windows",
