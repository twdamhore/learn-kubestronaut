...
var questions = [
  ...
  },
  {
    ...
    options: [
      ...
      "PVCs follow the pattern `<statefulset-name>-<ordinal>`, e.g. `db-0`, `db-1`, and `db-2` per replica"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "The kubelet periodically syncs mounted ConfigMap volumes, updating the projected files without a pod restart"
    ],
    answer: 3,
  ...
  },
  {
    ...
    options: [
      ...
      "MinIO — an S3-compatible object storage server designed for cloud-native data management",
      "Rook — a storage orchestrator that deploys and manages Ceph clusters running on Kubernetes",
  ...
  },
  {
    ...
    diagram: null,
    options: [
      "The pod can be scheduled to any node because the local volume on `worker-03` is replicated across nodes",
      ...
      "The pod is constrained to `worker-03` because local PVs include node affinity that restricts pod placement"
    ],
    answer: 3,
  ...
  },
  {
    ...
    diagram: null,
    options: [
      "`kubelet_volume_stats_inodes` paired with `kubelet_volume_stats_inodes_free` per volume",
      "`node_disk_io_time_seconds_total` paired with `node_disk_read_bytes_total` per device",
  ...
  },
  {
    ...
    diagram: null,
    options: [
      "PV-B, because Kubernetes selects the smallest PV that satisfies the PVC's request",
  ...
  },
  {
    ...
    options: [
      ...
      "`helm install db ./chart --env GCP_PROVISIONER=pd.csi.storage.gke.io` (exported)"
    ],
    answer: 0,
  ...
  },
  {
    ...
    options: [
      ...
      "Only `pod-3` and `pod-4` receive the new revision; pods below the partition value keep the old one",
  ...
  },
  {
    ...
    options: [
      ...
      "Retaining PVCs prevents data loss; when the replacement pod is created with the same ordinal it reattaches",
  ...
  },
  {
    ...
    diagram: null,
    options: [
      "The I/O throughput (`read_bytes`, `write_bytes`) of each PVC from the `kubelet_volume_stats` endpoint",
  ...
  },
  {
    ...
    options: [
      ...
      "The ordinal in the pod name correlates logs with specific replicas, helping debug per-instance storage issues",
